...
import {
  LoanInterest,
  LoanMinimumTransactionPercent,
  LoanStatus,
} from "../interfaces/enum/loan.enum";
...
class AccountController {
  ...
  async getAllUserAccounts(req: Request, res: Response) {
    try {
      const params = { ...req.body };
      let accounts = await this.accountService.getAccountByUserId(
        params.user.id
      );
      return Utility.handleSuccess(
        res,
  ...
  async applyLoan(req: Request, res: Response) {
    try {
      ...
      if (loanExists) {
        return Utility.handleError(
          res,
          "You already have a loan pending on this account",
          ResponseCode.NOT_FOUND
        );
      }
      ...
      console.log(minRequiredTransaction);
